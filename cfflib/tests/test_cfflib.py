...
def test_connectome():
    ...
    assert_equal(c.get_connectome_meta().name, 'myconnectome')
    ...
    assert_equal(c.get_connectome_meta().name, 'Test connectome')
...
def test_cnetwork_nxgraph():
    ...
    n = CNetwork()
    assert_equal(n.name, 'mynetwork')
    
    n = CNetwork('Test network')
    assert_equal(n.name, 'Test network')
    ...
    assert_equal(n.name, 'Test network')
    assert_equal(n.src, 'CNetwork/test_network.gpickle')
    assert_equal(n.dtype, 'AttributeNetwork')
    assert_equal(n.fileformat, 'NXGPickle')
    ...
    assert_equal(c.get_connectome_network()[1].name, '2nd nxgraph')
...
def test_save_load():
    ...
    assert_equal(c.get_connectome_meta().name, 'Load & Save connectome')
    assert_equal(c.get_connectome_meta().generator, 'cfflib')
    assert_equal(c.get_connectome_meta().version, '2.0')
    ...
    assert_equal(c2.get_connectome_meta().name, 'Load & Save connectome')
    assert_equal(c2.get_connectome_meta().generator, 'cfflib')
    assert_equal(c2.get_connectome_meta().version, '2.0')
    assert_equal(c2.get_connectome_network()[0].src, 'CNetwork/graphml_net.graphml')
